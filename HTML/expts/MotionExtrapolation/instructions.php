...
$path = $path."/MotionExtrapolation";
if(!is_dir($path)){
	// user's data folder corresponding to Tone Discrimination doesn't exist
	// make it
	$ret = mkdir($path, 0777);
	if (!$ret)
		die('Failed to create data folder');
...
<!-- <input type="hidden" name="discRadius" value="<?php echo $_REQUEST['discRadius'];?>"/>
<input type="hidden" name="velocity" value="<?php echo $_REQUEST['velocity'];?>"/>
<input type="hidden" name="pathRadius" value="<?php echo $_REQUEST['pathRadius'];?>"/>
<input type="hidden" name="pathLength" value="<?php echo $_REQUEST['pathLength'];?>"/> -->
